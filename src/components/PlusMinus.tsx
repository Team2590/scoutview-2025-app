...
export default function PlusMinus({ property, label }: { property: keyof Data, label: string }) {
    ...
    return (
        <div className='text-center'>
            ...
            <p className={`bg-${alliance == Alliance.RED ? 'red-alliance' : 'blue-alliance'}-info mx-auto py-3 rounded`} style={{ width: 156 }}>{data[property]}</p>
            <div className='btn-group' style={{ marginTop: -16 }}>
                ...
                <button
                    className={`btn btn-${alliance == Alliance.RED ? 'red-alliance' : 'blue-alliance'}-primary rounded-start-0`}
                    style={buttonStyles}
                    onClick={increment}
                >
                    +
